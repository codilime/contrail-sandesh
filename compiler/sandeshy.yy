 ...
 * Sandesh parser.
 *
 * This parser is used on a sandesh definition file.
 *
 * sandesh domain specific langauge is derived from apache thrift and
 * much of thrift code is reused.
 */
#endif
 ...
 */
%token tok_oneway
%token tok_staticfunction
%token tok_abcfunction
...
%type<tsandesh>  Sandesh
%type<ttype>     SandeshType
...
%type<tbool>     Oneway
%type<tbool>     StaticFunction
%type<tbool>     AbcFunction
%type<tbool>     XsdAll
...
| Sandesh
    {
      pdebug("Definition -> Sandesh");
      if (g_parse_mode == PROGRAM) {
        if (g_scope->get_sandesh($1->get_name())) {
          yyerror("Duplicate definition of sandesh: %s ", $1->get_name().c_str());
          exit(1);
        }
        g_scope->add_sandesh($1->get_name(), $1);
        if (g_parent_scope != NULL) {
          g_parent_scope->add_sandesh(g_parent_prefix + $1->get_name(), $1);
        }
        g_program->add_sandesh($1);
      }
      $$ = $1;
    }
...
SandeshType:
  tok_system
    {
      $$ = g_type_sandesh_system;
    }
|
  tok_request
    {
      $$ = g_type_sandesh_request;
    }
|
  tok_response
    {
      $$ = g_type_sandesh_response;
    }
|
  tok_trace
    {
      $$ = g_type_sandesh_trace;
    }
|
  tok_trace_object
    {
      $$ = g_type_sandesh_trace_object;
    }
|
  tok_buffer
    {
      $$ = g_type_sandesh_buffer;
    }
|
  tok_uve
    {
...
Function:
  CaptureDocText Oneway AbcFunction StaticFunction FunctionType tok_identifier '(' FieldList ')' Throws CommaOrSemicolonOptional
    {
      $8->set_name(std::string($6) + "_args");
      $$ = new t_function($5, $6, $8, $10, $2, $3, $4);
      if ($1 != NULL) {
        $$->set_doc($1);
      }
    }
...
SFieldList:
  SFieldList Field
    {
      pdebug("SFieldList -> SFieldList , Field");
      $$ = $1;
 ...
 CaptureDocText FieldIdentifier tok_literal CommaOrSemicolonOptional 
    {
      ...
      $$ = new t_field(g_type_static_const_string, t_field::auto_string_name($2.value), $2.value);

      $$->set_req(t_field::T_REQUIRED);
      $$->set_value(new t_const_value($3));
      if ($1 != NULL) {
        $$->set_doc($1);
      }
    }
